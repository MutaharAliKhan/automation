...
root = tk.Tk()
root.title("Script Parser")
root.geometry("400x100")

tk.Label(root, text="Destination folder:").grid(row=0, column=0, sticky=tk.W)
destination_folder_field = tk.Entry(root, width=50)
destination_folder_field.insert(0, 'system_generated_scripts')
destination_folder_field.grid(row=0, column=1, columnspan=3)


def generate():
    destination_folder = destination_folder_field.get().strip('"').strip("'")
    if not destination_folder:
        messagebox.showerror('Error', 'Error: Destination folder is required.', bg='#003366',
                             fg='#ffffff')
        return
    runner(destination_folder)
    root.destroy()


tk.Button(root, text="Generate", command=generate).grid(row=1, column=1, columnspan=2)
