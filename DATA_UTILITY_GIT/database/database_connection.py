...
def database_connection_all(connection_string, query, values=None):
    start_time = time.time()
    try:
        with pyodbc.connect(connection_string) as connection:
            connection.autocommit = True
...
def update_connection_string(connection_branch=None):
    env_file_path = get_file_location('env_config.json', file_extension=".json")
...
def execute_query(connection_branch, query, values=None):
    ...
    data = database_connection_all(connection_string, query, values)

    if isinstance(data, list) and len(data) > 0:
        table_name = query.split()[3]  # Simple way to get table name; adjust as needed
        write_to_csv(data, table_name)
