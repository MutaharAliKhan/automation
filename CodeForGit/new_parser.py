    ...
    class FillAndGetByTextVisitor(ast.NodeVisitor):
        ...
        def get_string_value(self, node):
            if isinstance(node, ast.Str):
                ...
                # Handle the case where the value is a result of a function call
                func_name = self.get_function_name(node.func)
                if func_name:
                    # Add logic to handle different function calls if needed
        ...
        def visit_Call(self, node):
            ...
            if isinstance(node.func, ast.Attribute):
                ...
                if method_name == 'fill':
                    ...
                    if value_node:
                        locator_str = ast.get_source_segment(script_content, node.func.value.args[0]) if isinstance(locator_node, ast.Call) else ast.get_source_segment(script_content, node.args[0])
...
script_path = r'D:\MyRecentProjects\Data_Utility\scripting.py'
