...
def add_method(file_path, method_name, column_values):
    key_value_pairs = convert_list_to_dict(column_values[0], column_values[1])
...
def generate_database_operation_method(csv_path, script_name):
    ...
    db_operations_path = os.path.join('database_operations.py')
...
def extract_page_lines_and_comments1(script_content, exclude_keywords=None):
    ...
    for node in ast.walk(tree):
        # Extract lines with `page.` actions
    ...
    # If multiline_comment_lines contains any comments, add them to multi_line_comments
    if multiline_comment_lines.strip():
        multi_line_comments.append((None, None, multiline_comment_lines.strip()))
...
def process_script_with_ast(script_content_path, updated_script_path, csv_path, script_name):
    try:
        ...
        page_lines, single_line_comments, multi_line_comments, excluded_lines = extract_page_lines_and_comments(script_content)
...
def process_directory(input_dir, output_dir):
    """
    Walks through the input directory and its subdirectories,
    processes all .py files, and saves updated scripts and corresponding
    CSV files to the output directory while maintaining the same directory structure.
    """
