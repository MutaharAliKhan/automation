...
def parse_scripts(golden_script_dir):
    try:
        script_directory = os.path.dirname(os.path.abspath(__file__))
        project_root_directory = os.path.abspath(os.path.join(script_directory, '..'))
        log_dir = os.path.join(project_root_directory, 'logs')
        ...
        golden_scripts = os.path.join(project_root_directory, golden_script_dir)
        system_generated_scripts = os.path.join(project_root_directory, 'system_generated_scripts')
        os.makedirs(system_generated_scripts, exist_ok=True)
...
"""
            ...
            try:
                ...
                imports_needed = f"""import pandas as pd
import random
import socket
from database.Queries import *
from database.database_connection import *
from utils.random_data_provider import * \n
test_data = read_json(r"{os.path.join(project_root_directory, 'database', 'test_data.json')}")
...
"""
        ...
        def process_script_with_ast(script_path, updated_script_path, csv_path, script_name):
            ...
            try:
                ...
                db_operations_path = os.path.join(project_root_directory, 'database', 'database_operations.py')
...
"""
        ...
        for script_file in os.listdir(golden_scripts):
            script_path = os.path.join(golden_scripts, script_file)

            if os.path.isfile(script_path):
                script_name = os.path.splitext(script_file)[0]
                updated_script_path = os.path.join(system_generated_scripts, script_file)
                csv_path = os.path.abspath(os.path.join(system_generated_scripts, f'{script_name}.csv'))
